...
from last30free.notification_store import (
...
def test_notification_store_saves_bundle_and_lists_history(tmp_path) -> None:
    ...
    bundle = build_notification_bundle(alert_report, channels=["email", "webhook"])
    saved = write_notification_bundle(output_dir, bundle)

    assert Path(saved["bundle_path"]).exists()
    assert Path(saved["manifest_path"]).exists()
    assert Path(saved["email_payload_path"]).exists()
    assert Path(saved["email_body_path"]).exists()
    assert Path(saved["webhook_payload_path"]).exists()
    assert notification_index_path(output_dir).exists()

    history = list_notification_snapshots(output_dir, topic_ref="ai coding tools")
    assert len(history) == 1
    assert history[0]["topic"] == "ai coding tools"
    assert history[0]["triggered_rules"] >= 1
